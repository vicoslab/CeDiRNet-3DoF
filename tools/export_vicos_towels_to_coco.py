...
def convert_to_coco_format(subfolders, output_file, R = 25):
    ...
    coco_format = {
        "info": {                
            "version": "v2", 
            "description": "Vicos Towels Dataset",
            "contributor": "domen.tabernik@fri.uni-lj.si", 
            "url": "", 
            "year": datetime.now().year,
            "date_created": datetime.now().strftime("%Y-%m-%d"),
        },
        "categories": [{"supercategory": CAT_CENTER_NAME, "id": CAT_CENTER_ID, "name": CAT_CENTER_NAME}],
        "licenses": [],
        "images": [],
        "annotations": [],        
    }
...
if __name__ == "__main__":
    ...
    convert_to_coco_format(train_subfolders, output_file=os.path.join(ROOT_DIR, 'coco_train.json'))
    convert_to_coco_format(test_subfolders, output_file=os.path.join(ROOT_DIR, 'coco_test.json'))

    convert_to_coco_format(test_novel_bg_subfolders, output_file=os.path.join(ROOT_DIR, 'coco_test_novel=bg.json'))
    convert_to_coco_format(test_novel_cloth_subfolders, output_file=os.path.join(ROOT_DIR, 'coco_test_novel=cloth.json'))
    convert_to_coco_format(test_novel_cloth_and_bg_subfolders, output_file=os.path.join(ROOT_DIR, 'coco_test_novel=bg+cloth.json'))
