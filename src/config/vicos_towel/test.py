...
args = dict(
	...
	eval=dict(
		...
		orientation=dict(
			...
			tau_thr=[20], # [5,10,20] # try with 5px, 10px and 20px limit
		),
	...
	dataset={
		...
		'kwargs': {
			...
			'subfolders': [dict(folder='bg=red_tablecloth', data_subfolders=['cloth=checkered_rag_small', 'cloth=cotton_napkin']),
						   ...
						   dict(folder='bg=festive_tablecloth', data_subfolders=['cloth=big_towel','cloth=checkered_rag_big','cloth=checkered_rag_medium', 'cloth=checkered_rag_small', 'cloth=cotton_napkin', 'cloth=linen_rag','cloth=small_towel','cloth=towel_rag','cloth=waffle_rag','cloth=waffle_rag_stripes'])
						   ],
